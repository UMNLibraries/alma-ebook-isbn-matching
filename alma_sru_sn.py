...
def sn_sru(fname):
	...
	'''
	Searches a list of ISBNs in Alma's standard number index via SRU. Writes a tab-delimited
	.txt file of found ISBNs with Alma MMS IDs.	Returns two counts: number of ISBNs searched, 
	...
	with open(fname) as infile:
		...
		for isbn in isbns:
			...
			search_url = f'https://na01.alma.exlibrisgroup.com/view/sru/REDACTED_INSTITUTION_CODE?version=1.2&operation=searchRetrieve&recordSchema=marcxml&query=alma.standard_number={isbn} and alma.mms_tagSuppressed=false'
		...
		df_results.to_excel('isbns_found_all.xlsx')

	with open("isbns_found.txt", "w+") as foundfile:
		for k, v in foundlist.items():
			foundfile.write(str(k) + '\t' + str(v) + '\n')
